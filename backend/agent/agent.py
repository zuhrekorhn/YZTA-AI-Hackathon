    ...
    for _ in range(max_iterations):
        try:
            ...
            if _is_quota_error(exc):
                return "AI servisi şu anda yoğun veya kota sınırına ulaştı. Lütfen birkaç saniye sonra tekrar deneyin."
            raise
...
def run_customer_agent(
    ...
    save_message(db, business_id, session_id, "customer", user_message, message_type="customer_to_ai")
    # Persist customer display name once per session for manager view
    try:
        from repositories.message_repo import get_customer_name_for_session
        existing = get_customer_name_for_session(db, business_id, session_id)
        if customer_name and customer_name.strip() and customer_name != "Müşteri" and not existing:
            save_message(db, business_id, session_id, "customer_name", customer_name.strip(), message_type="customer_to_ai")
    except Exception:
        # non-fatal if helper import or save fails
        pass
    ...
    save_message(db, business_id, session_id, "ai", response, message_type="customer_to_ai")
    return response

def run_manager_ask_ai(
    db: Session,
    business_id: str,
    session_id: str,
    user_message: str
) -> str:
    """Manager asks AI separately (not in customer chat context)"""
    save_message(db, business_id, session_id, "manager", user_message, message_type="manager_to_ai")
    history = _build_history(
        get_messages_by_session(db, session_id, business_id, limit=10)
    )
    ...
    save_message(db, business_id, session_id, "ai", response, message_type="manager_to_ai")
    return response

def run_manager_to_customer(
    db: Session,
    business_id: str,
    session_id: str,
    message_content: str
    ...
    save_message(db, business_id, session_id, "manager", message_content, message_type="manager_to_customer")