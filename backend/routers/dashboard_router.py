...
from repositories.order_repo import get_orders_by_business, create_order
from services.analytics_service import predict_stockout
from schemas.schemas import StockUpdate, NewOrder
from schemas.schemas import NotifyRequest, NotifyResponse
from models.tables import Product, Order
from datetime import date
from agent.tools import tool_notify_customers
...
def add_order(
    ...
    order = create_order(db, business_id, data.dict())
    return {
        "id": order.id,
...
def notify_orders(
    ...
    result = tool_notify_customers(db=db, business_id=business_id, order_ids=data.order_ids, message_type=data.message_type)
    return result